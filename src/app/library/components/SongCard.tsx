...
function SongCardComponent({
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-2xl bg-[#F9FAFB] transition hover:shadow-sm">
      <div className="relative h-44 w-full bg-[#E5E7EB]">
        ...
        <div className="absolute left-3 top-3 flex items-center gap-2 text-xs">
          {song.status && (
            <span className={`rounded-md px-2 py-1 text-xs font-semibold uppercase tracking-wider text-white ${
              song.status === 'lyrics_ready' ? 'bg-[#84CC16]/80' :
              song.status === 'ready' ? 'bg-[#84CC16]/80' :
              song.status === 'generating_lyrics' || song.status === 'generating_music' ? 'bg-[#A855F7]/80' :
              'bg-[#6B7280]/80'
            }`}>
              {song.status === 'lyrics_ready' ? 'Ready to Choose' :
               song.status === 'ready' ? 'Ready' :
               song.status === 'generating_lyrics' ? 'Generating' :
               song.status === 'generating_music' ? 'Generating' :
               song.status}
            </span>
          )}
          {song.isPublic ? (
            <span className="rounded-md bg-[#84CC16]/80 px-2 py-1 text-xs font-semibold text-white">
              Gedeeld
            </span>
          ) : null}
        </div>
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        <div>
          ...
          {snippet && <p className="mt-1 text-sm text-[#6B7280] line-clamp-3">{snippet}</p>}
        </div>
        ...
        {variants.length > 1 && (
          <label className="text-xs font-medium text-[#6B7280]">
            Variant
            <select
              className="mt-1 w-full rounded-lg border border-[#E5E7EB] bg-white px-2 py-1 text-sm text-[#262626]"
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="rounded-full bg-[#84CC16] px-4 py-2 text-xs font-semibold text-white transition hover:bg-[#65A30D] disabled:cursor-not-allowed disabled:opacity-60"
          >
          ...
          {primaryCTA.action !== 'view_details' && (
            <button
              type="button"
              onClick={onOpen}
              className="rounded-full border border-[#E5E7EB] px-3 py-2 text-xs font-semibold text-[#6B7280] transition hover:border-[#D1D5DB] hover:text-[#262626]"
            >
              Open
            </button>
          )}
          ...
          {song.status === 'ready' && (
            <button
              ...
              className="rounded-full border border-[#E5E7EB] px-3 py-2 text-xs font-semibold text-[#6B7280] transition hover:border-[#D1D5DB] hover:text-[#262626] disabled:cursor-not-allowed disabled:opacity-60"
            >
              {actionState?.isSharing ? "Bezig…" : song.isPublic ? "Link" : "Deel"}
            </button>
          )}
          ...
          <button
            ...
            className="ml-auto rounded-full border border-[#FEE2E2] px-3 py-2 text-xs font-semibold text-[#DC2626] transition hover:border-[#FECACA] hover:bg-[#FEF2F2] disabled:cursor-not-allowed disabled:opacity-60"
          >
            {actionState?.isDeleting ? "..." : "×"}
          </button>
        </div>

        <div className="mt-auto text-xs text-[#6B7280]">
          {metadataText}
        </div>
      </div>
    </div>
